...
Returns an association list containing the following elements:
...
(defun calendar-remind-print (&optional day mon year)
  "Execute the `remind' program and print the results into the
current buffer.
\n`calendar-current-date' is used to determine the date."
  (interactive "*")
...
(defun calendar-remind-lookup (&optional day mon year)
  "Execute the `remind' program and output the results into the
`calendar-remind-buffer-name' buffer.
\nUses `calendar-remind-print' to output the remind information."
  (interactive)
...
(defun calendar-remind-visit (&optional file-name)
  "Edit file FILE-NAME using `find-file'.
\nFILE-NAME is the file to edit (defaults to
...
(defun calendar-remind-visit-insert (&optional file-name)
  "Call `calendar-remind-visit' then insert a new entry with the
selected date."
  (interactive)
...
Used on an entry in a `calendar-remind-buffer-name' buffer."
  (interactive)
...
;; (when (not (functionp 'calendar-remind-load-hook))
